...
if (!function_exists('getPermission')) {
    function getPermission($permissions, $moduleId): array
    {
        return Arr::first($permissions, static function ($value) use ($moduleId) {
                return ($value->module_id == $moduleId);
            })?->permissions ?? [];
    }
}
...
if (!function_exists('getOrganizationId')) {
    function getOrganizationId(): ?string
    {
        if (!app()->runningInConsole()) {
            return config('app.organization_id');
        }
