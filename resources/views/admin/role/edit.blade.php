<x-layout title="Edit Role">
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12">
            <!-- BEGIN: Form Layout -->
            <form action="{{ route('userManagement.role.update',$role->id) }}" method="post"
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        ...
                        <div class="w-full md:w-1/4 px-3 mb-4">
                            ...
                            <input id="expiry_date" type="text" class="datepicker form-control"
                                   ...
                                   value="{{ old('expiry_date', $role->expiry_date) }}"/>
                            @error('expiry_date')
                            <span class="text-danger" role="alert">
                            {{ $message }}
                        </span>
                        ...
                        <div class="w-full md:w-1/4 px-3 mb-4">
                            ...
                            <div class="form-switch mt-2">
                                <input type="checkbox" id="is_active" class="form-check-input"
                                       ...
                                       @if(old('is_active', $role->is_active) == '1') checked @endif>
                                <input type="hidden" name="is_active"
                                       value="{{ (old('is_active', $role->is_active) == '1') ? "1" : "0" }}"
                                       id="activeHidden">
                            </div>
                            @error('active')
                            <span class="text-danger" role="alert">
                                        ...
                                        <div class="">
                                            <table class="table table-striped">
                                                ...
                                                @foreach($module->children as $children)
                                                    @php $permissions = getPermission($role->rolePermissions, $children->id); @endphp
                                                    <tr>
                                                        <td>{{ $children->name }}</td>
                                                        <td style="width: 75%">
                                                            <select data-placeholder="Select"
                                                                ...
                                                                @foreach($children->permissions as $perKey => $permission)
                                                                    <option
                                                                        @selected(in_array($perKey,$permissions,true))
